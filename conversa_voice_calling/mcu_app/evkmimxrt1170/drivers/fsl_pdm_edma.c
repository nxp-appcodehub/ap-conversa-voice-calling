...
#include "appGlobal.h" // required due to issue with PDM Mic setup TODO SDK UPDATE
 ...
 * param g_pdmConfig pdm channel configurations.
 */
                                      ...
                                      const pdm_channel_config_t *config)
{
    ...
    //TODO SDK UPDATE Reported internally to SDK team
    handle->channelNums = APP_AUDIO_PDM_CHANNEL_NUMBER_MAX; // always read the 8 channels as the EDMA is not able to detect witch channel is enable or not
 ...
 * This function support dynamic scatter gather and static scatter gather,
 ...
 * before the previous one finish.
 * b. for the static scatter gather case: Application should use the link transfer
...
status_t PDM_TransferReceiveEDMA(PDM_Type *base, pdm_edma_handle_t *handle, pdm_edma_transfer_t *xfer)
{
    ...
    //TODO SDK UPDATE always start to channel 0
     uint32_t startAddr            = PDM_GetDataRegisterAddress(base, 0);
    ...
    while (currentTransfer != NULL)
    {
        ...
        else
        {
            EDMA_PrepareTransferConfig(
            						  &config,
									  (void *)(uint32_t *)startAddr,
									  FSL_FEATURE_PDM_FIFO_WIDTH,
									  FSL_FEATURE_PDM_FIFO_OFFSET,
									  (uint8_t *)(uint32_t)currentTransfer->data,
									  FSL_FEATURE_PDM_FIFO_WIDTH,
									  FSL_FEATURE_PDM_FIFO_OFFSET,
									  handle->channelNums * (uint32_t)FSL_FEATURE_PDM_FIFO_WIDTH,
									  currentTransfer->dataSize);
        }
    ...
    if (handle->state != (uint32_t)kPDM_Busy)
    {
		...
		//SDK UPDATE start later for synchro with RX flow
