...
//---??? these baud rate settings has no effect???????
//#define LINE_CODING_DTERATE (115200)
//#define LINE_CODING_DTERATE (115200/8)
#define LINE_CODING_DTERATE (1200)
...
typedef struct _usb_cdc_acm_info
{
	...
	extern  uint8_t VComSendBuf[DATA_BUFF_SIZE*4+64];		//each time VCOM send upstream audio data --- 64 samples *6 ch * 4 bytes = 3 sectors
#else
	extern  uint8_t VComSendBuf[DATA_BUFF_SIZE+64];
#endif
	extern volatile uint32_t VCom_sendSize;
