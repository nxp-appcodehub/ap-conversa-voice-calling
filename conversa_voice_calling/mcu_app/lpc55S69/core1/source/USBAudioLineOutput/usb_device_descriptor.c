...
usb_device_endpoint_struct_t g_UsbDeviceAudioRecorderEndpoints[USB_AUDIO_RECORDER_STREAM_ENDPOINT_COUNT] = {
    /* Audio generator ISO IN pipe */
    {
        ...
        0,//to be filled with right value before USB init
...
uint8_t g_UsbDeviceConfigurationDescriptor[] = {
    ...
    USB_SHORT_GET_LOW(
				...
				//UAC --- interface 0 --- alternate 0
				USB_IAD_DESC_SIZE +
				USB_DESCRIPTOR_LENGTH_INTERFACE +
					USB_AUDIO_CONTROL_INTERFACE_HEADER_LENGTH + USB_AUDIO_CLOCK_SOURCE_LENGTH + USB_AUDIO_CLOCK_SOURCE_LENGTH + 0x11U + 0x12U +
					0x0CU + +0x11U + 0x12U + 0x0CU +
				//UAC --- interface 1 --- alternate 0, 1
				USB_DESCRIPTOR_LENGTH_INTERFACE +
					USB_DESCRIPTOR_LENGTH_INTERFACE + 0x10U + 0x06U + USB_AUDIO_STREAMING_ENDP_DESC_SIZE + USB_AUDIO_CLASS_SPECIFIC_ENDPOINT_LENGTH +
				//UAC --- interface 2 --- alternate 0, 1
				USB_DESCRIPTOR_LENGTH_INTERFACE +
					USB_DESCRIPTOR_LENGTH_INTERFACE + 0x10U + 0x06U + USB_AUDIO_STREAMING_ENDP_DESC_SIZE + USB_AUDIO_CLASS_SPECIFIC_ENDPOINT_LENGTH +
					#if defined(USB_DEVICE_AUDIO_USE_SYNC_MODE) && (USB_DEVICE_AUDIO_USE_SYNC_MODE > 0U)
					#else
					...
					#endif
				//CDC --- interface 3 and 4
				USB_IAD_DESC_SIZE +
				USB_DESCRIPTOR_LENGTH_INTERFACE +
					USB_DESCRIPTOR_LENGTH_CDC_HEADER_FUNC + USB_DESCRIPTOR_LENGTH_CDC_CALL_MANAG + USB_DESCRIPTOR_LENGTH_CDC_ABSTRACT + USB_DESCRIPTOR_LENGTH_CDC_UNION_FUNC + USB_DESCRIPTOR_LENGTH_ENDPOINT +
				USB_DESCRIPTOR_LENGTH_INTERFACE +
...
//-----next is for audio class
    ...
    USB_AUDIO_CONTROL_INTERFACE_INDEX,                /* The first interface number associated with this function is 0   */
    USB_AUDIO_COMPOSITE_INTERFACE_COUNT,              /* The number of contiguous interfaces associated with this function is 2   */
   ...
   // 0x6F, 0x00U,  /* Total number of bytes returned for the class-specific
    0x77, 0x00U,  /* Total number of bytes returned for the class-specific  UPDATE to fix descriptor issue
    ...
    USB_AUDIO_CONTROL_RECORDER_CLOCK_SOURCE_ENTITY_ID, /* Constant uniquely identifying the Clock Source Entity within the audio
                                               * funcion
                                               */
...
#else
	0x05U,0x04U,    // recognized as a Echo-cancelling Speakerphone
	0x01U,          // associate to number 1 => output interface should be defined also as echo-cancelling speakerphone with the same association number
#endif
    USB_AUDIO_CONTROL_RECORDER_CLOCK_SOURCE_ENTITY_ID, /* ID of the Clock Entity to which this Input Terminal is connected.  */
    ...
    USB_AUDIO_CONTROL_RECORDER_CLOCK_SOURCE_ENTITY_ID,   /* ID of the Clock Entity to which this Output Terminal is connected  */
    0x00U, 0x00U,                               /* bmControls:   D1..0: Copy Protect Control is not present
                                                   D3..2: Connector Control is not present
                                                   D5..4: Overload Control is not present
    ...
    0x01U, /* A Terminal dealing with a signal carried over an endpoint in an AudioStreaming interface. The
              AudioStreaming interface descriptor points to the associated Terminal through the bTerminalLink field. */
    0x00U, /* This Input Terminal has no association   */
    USB_AUDIO_CONTROL_SPEAKER_CLOCK_SOURCE_ENTITY_ID, /* ID of the Clock Entity to which this Input Terminal is connected.  */
    ...
    USB_AUDIO_SPEAKER_CONTROL_OUTPUT_TERMINAL_ID,         /* Constant uniquely identifying the Terminal within the audio
                      function. This value is used in all requests         to address this Terminal.   */
#if USB_MIC_ONLY
	0x00U,0x03U,    // undefined output => it will be automatically disable by windows
	0x00U,          // no association
#else
	0x05U,0x04U,    // recognized as a Echo-cancelling Speakerphone
	0x01U,          // associate to number 1 => input interface should be defined also as echo cancelling speakerphone with the same association number
#endif
    USB_AUDIO_SPEAKER_CONTROL_FEATURE_UNIT_ID, /* ID of the Unit or Terminal to which this Terminal is connected.  */
    USB_AUDIO_CONTROL_SPEAKER_CLOCK_SOURCE_ENTITY_ID,  /* ID of the Clock Entity to which this Output Terminal is connected  */
    0x00U, 0x00U,                              /* bmControls:   D1..0: Copy Protect Control is not present
                                                  D3..2: Connector Control is not present
                                                  D5..4: Overload Control is not present
                                                  D7..6: Underflow Control is not present
	...
	//AUDIO_IN_FORMAT_SIZE*8,                   /* The number of effectively used bits from the available bits in an audio subslot   */
    0,//to be filled with right value before USB init
	...
	//AUDIO_OUT_FORMAT_SIZE*8,                   /* The number of effectively used bits from the available bits in an audio subslot   */
    0,//to be filled with right value before USB init
    0,//to be filled with right value before USB init
...
USB_DMA_INIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
uint8_t g_UsbDeviceString3[] = {2U + 2U * 4U, USB_DESCRIPTOR_TYPE_STRING,
                                '1',           0x00U,
                                '2',           0x00U,
                                '3',           0x00U,
                                '4',           0x00U};
...
PL_UINT32 g_UsbDeviceStringDescriptorLength[USB_DEVICE_STRING_COUNT] = {
...
usb_status_t USB_DeviceSetSpeed(usb_device_handle handle, uint8_t speed)
{
        ...
        if (descriptorHead->common.bDescriptorType == USB_DESCRIPTOR_TYPE_ENDPOINT)
        {
            if (USB_SPEED_HIGH == speed)
            {
                     ...
                     USB_OUT))
                {
                    ...
                    USB_SHORT_TO_LITTLE_ENDIAN_ADDRESS((g_UsbCompositeDevPtr->HS_ISO_OUT_ENDP_PACKET_SIZE),
                           ...
                           USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT) == USB_IN))
                {
                    ...
                    USB_SHORT_TO_LITTLE_ENDIAN_ADDRESS((g_UsbCompositeDevPtr->HS_ISO_IN_ENDP_PACKET_SIZE),
                                                       descriptorHead->endpoint.wMaxPacketSize);
                }
#else
                if ((USB_AUDIO_SPEAKER_STREAM_ENDPOINT ==
            ...
            else
            {
                     ...
                     USB_OUT))
                {
                    ...
                    USB_SHORT_TO_LITTLE_ENDIAN_ADDRESS((g_UsbCompositeDevPtr->FS_ISO_OUT_ENDP_PACKET_SIZE),
                                                       descriptorHead->endpoint.wMaxPacketSize);
                }
                else if ((USB_AUDIO_RECORDER_STREAM_ENDPOINT ==
                          (descriptorHead->endpoint.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK)) &&
                           ...
                           USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT) == USB_IN))
                {
                    ...
                    USB_SHORT_TO_LITTLE_ENDIAN_ADDRESS((g_UsbCompositeDevPtr->FS_ISO_IN_ENDP_PACKET_SIZE),
                                                       descriptorHead->endpoint.wMaxPacketSize);
                }
#else
                if ((USB_AUDIO_SPEAKER_STREAM_ENDPOINT ==
    ...
    //for (PL_INT32 i = 0U; i < USB_AUDIO_ENDPOINT_COUNT; i++)
    {
        if (USB_SPEED_HIGH == speed)
        {
			#if defined(USB_DEVICE_AUDIO_USE_SYNC_MODE) && (USB_DEVICE_AUDIO_USE_SYNC_MODE > 0U)
				g_UsbDeviceAudioSpeakerEndpoints[0].maxPacketSize = (g_UsbCompositeDevPtr->HS_ISO_OUT_ENDP_PACKET_SIZE);
        ...
        {
			#if defined(USB_DEVICE_AUDIO_USE_SYNC_MODE) && (USB_DEVICE_AUDIO_USE_SYNC_MODE > 0U)
				g_UsbDeviceAudioSpeakerEndpoints[0].maxPacketSize = (g_UsbCompositeDevPtr->FS_ISO_OUT_ENDP_PACKET_SIZE);
...
#if 1
    for (PL_INT32 i = 0; i < USB_CDC_VCOM_CIC_ENDPOINT_COUNT; i++)
    {
        if (USB_SPEED_HIGH == speed)
        {
    ...
    for (PL_INT32 i = 0; i < USB_CDC_VCOM_DIC_ENDPOINT_COUNT; i++)
    {
        if (USB_SPEED_HIGH == speed)
        {
...
void FillUsbDescriptorRelatedValuesBeforeUsbInit(void)
{
    usb_descriptor_union_t *descriptorHead;
    usb_descriptor_union_t *descriptorTail;
	...
	g_UsbDeviceAudioRecorderEndpoints[0].maxPacketSize=	g_UsbCompositeDevPtr->FS_ISO_IN_ENDP_PACKET_SIZE + g_UsbCompositeDevPtr->AUDIO_IN_FORMAT_CHANNELS *g_UsbCompositeDevPtr->AUDIO_IN_FORMAT_SIZE;
	g_UsbDeviceAudioSpeakerEndpoints[0].maxPacketSize=  g_UsbCompositeDevPtr->FS_ISO_OUT_ENDP_PACKET_SIZE;
    ...
    {
    	//11 24 ...
    	if ((descriptorHead->common.bLength == 0x11)&&(descriptorHead->common.bDescriptorType == 0x24))
    	{
    		if((descriptorHead->common.bData[0]==0x02)&&(descriptorHead->common.bData[1]==0x01))
    		{
    			descriptorHead->common.bData[6]=g_UsbCompositeDevPtr->AUDIO_IN_FORMAT_CHANNELS;
    		}
    		if((descriptorHead->common.bData[0]==0x02)&&(descriptorHead->common.bData[1]==0x04))
    		{
    			descriptorHead->common.bData[6]=g_UsbCompositeDevPtr->AUDIO_OUT_FORMAT_CHANNELS;
    		}
    	}
    	//10 24 ...
    	if ((descriptorHead->common.bLength == 0x10)&&(descriptorHead->common.bDescriptorType == 0x24))
    	{
    		if((descriptorHead->common.bData[0]==0x01)&&(descriptorHead->common.bData[1]==0x03))
    		{
    			descriptorHead->common.bData[8]=g_UsbCompositeDevPtr->AUDIO_IN_FORMAT_CHANNELS;
    		}
    		if((descriptorHead->common.bData[0]==0x01)&&(descriptorHead->common.bData[1]==0x04))
    		{
    			descriptorHead->common.bData[8]=g_UsbCompositeDevPtr->AUDIO_OUT_FORMAT_CHANNELS;
    		}
    	}
    	//06 24 ...
    	if ((descriptorHead->common.bLength == 0x06)&&(descriptorHead->common.bDescriptorType == 0x24))
    	{
    		static unsigned char idx=0;
    		if(idx==0)
    		{
    			//first time seeing this 06 24 sub descriptor
    			descriptorHead->common.bData[2]=g_UsbCompositeDevPtr->AUDIO_IN_FORMAT_SIZE;
    			descriptorHead->common.bData[3]=g_UsbCompositeDevPtr->AUDIO_IN_FORMAT_SIZE*8;
    		}
    		if(idx==1)
    		{
    			//second time seeing this 06 24 sub descriptor
    			descriptorHead->common.bData[2]=g_UsbCompositeDevPtr->AUDIO_OUT_FORMAT_SIZE;
    			descriptorHead->common.bData[3]=g_UsbCompositeDevPtr->AUDIO_OUT_FORMAT_SIZE*8;
    		}
			idx++;
    	}
