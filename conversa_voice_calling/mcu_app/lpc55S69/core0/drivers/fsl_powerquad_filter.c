...
// DRIVER UPDATE - Remove when Biquad filter is working correctly
int32_t check_PQ_error_internal(void)
{
	if((((POWERQUAD->ERRSTAT) & POWERQUAD_EVENTEN_EVENT_NAN_MASK) == POWERQUAD_EVENTEN_EVENT_NAN_MASK))
	{
		POWERQUAD->ERRSTAT = 0;
		//__NOP();
		return 1;
...
void PQ_BiquadCascadeDf2F32(const pq_biquad_cascade_df2_instance *S, float *pSrc, float *pDst, uint32_t blockSize)
{
	...
	// FIXME - Remove when Biquad filter is working correctly
	if(check_PQ_error_internal())
	{
		// Set output to zero
		memset(pDst, 0, sizeof(float)*blockSize);
		for (int32_t i = 0; i < S->numStages; i++){
			S->pState[i].param.v_n = 0;
			S->pState[i].param.v_n_1 = 0;
			S->pState[i].compreg = 0;
		}
	}
